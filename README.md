...
## Executing Program
After building the virtual environment, and put csv files in `RHA-1_pipeline/input/`
```bash
...
`OG-02, OG-06, OG-10, OG-14, OG-04, OG-08, OG-12, OG-16, OG-03, OG-07` `RHA-1_pipeline/input/` 

Rename them appropriately, for example: `OG-01.fastq.gz`

Then, modify the `RHA-1_pipeline/run_generate_fig.sh` script by changing: 
```diff
- preprocess=False
+ preprocess=True
```
...
```bash
cd RHA-1_pipeline/output/example/
...
Only selected scripts or modules have been retained under `RHA-1_pipeline/sRNAanalyst/`, and modifications may have been made for compatibility or customization.
